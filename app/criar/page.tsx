...
<button onClick={()=>{setF({...f,logoPos:"centro"})}} style={{padding:'6px',borderRadius:'8px',background:f.logoPos==="centro"?"#00FF88":"#222",color:f.logoPos==="centro"?"black":"white",fontSize:'9px'}}>Centro Embaixo</button>
<button onClick={()=>{setF({...f,logoPos:"esquerda"})}} style={{padding:'6px',borderRadius:'8px',background:f.logoPos==="esquerda"?"#00FF88":"#222",color:f.logoPos==="esquerda"?"black":"white",fontSize:'9px'}}>Esquerda</button>
<button onClick={()=>{setF({...f,logoPos:"direita"})}} style={{padding:'6px',borderRadius:'8px',background:f.logoPos==="direita"?"#00FF88":"#222",color:f.logoPos==="direita"?"black":"white",fontSize:'9px'}}>Direita</button>
</div>
...
<b style={{fontSize:'10px'}}>CAPA COM VIDEO MP4 RODANDO ATRAS</b>
...
<input style={inp} placeholder="Endereço ou link Maps ex: Holambra SP" value={f.loc} onChange={(e)=>{setF({...f,loc:e.target.value})}}/>
{f.loc? <div style={{height:'140px',marginTop:'8px',borderRadius:'12px',overflow:'hidden',border:'1px solid #333'}}><iframe src={getEmbedUrl(f.loc)} style={{width:'100%',height:'100%',border:'none'}} loading="lazy" title="mapa"/></div> : null}
...
{f.loc? <div style={{background:'white',borderRadius:'16px',overflow:'hidden'}}><iframe src={getEmbedUrl(f.loc)} style={{width:'100%',height:'140px',border:'none'}} title="maps2"/><div style={{padding:'8px',color:'#1a73e8',fontWeight:900,fontSize:'12px'}}>Abrir no Google Maps</div></div> : null}
</div>
</div>
</div>
</div>
