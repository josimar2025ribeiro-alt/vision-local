...
<h2 style={{fontWeight:900,fontSize:'12px'}}>ESQUERDA - EDITOR COMPLETO</h2>
...
<div style={card}><b style={{fontSize:'10px'}}>🖼️ CAPA - FOTO ou VÍDEO + NOME + DESCRIÇÃO</b><br/><input type="file" accept="image/*,video/*" onChange={async e=>{const f2=e.target.files?.[0]; if(f2){const u=await upload(f2,"capas"); if(u) setF({...f,foto:u})}}}/><input style={{...inp,marginTop:'8px'}} placeholder="Nome da Capa ex: Oficina do João" value={f.capaTitulo} onChange={e=>setF({...f,capaTitulo:e.target.value})}/><textarea style={{...inp,height:'50px'}} placeholder="Descrição da capa..." value={f.capaDesc} onChange={e=>setF({...f,capaDesc:e.target.value})}/></div>

<div style={{background:'#111',padding:'10px',borderRadius:'10px',marginBottom:'10px'}}>
<b style={{fontSize:'10px'}}>🔗 LINKS E BOTÕES DO BIOSITE</b>
<input style={inp} placeholder="🛠️ Link Nossos Serviços (ex: WhatsApp com lista)" value={f.serv} onChange={e=>setF({...f,serv:e.target.value})}/>
<input style={inp} placeholder="📚 Link Catálogo / Produtos (PDF ou site)" value={f.cat} onChange={e=>setF({...f,cat:e.target.value})}/>
<input style={inp} placeholder="📅 Link Agendamento" value={f.agend} onChange={e=>setF({...f,agend:e.target.value})}/>
<input style={inp} placeholder="💬 WhatsApp ex: 199..." value={f.wpp} onChange={e=>setF({...f,wpp:e.target.value})}/>
<input style={inp} placeholder="📸 Instagram https://..." value={f.insta} onChange={e=>setF({...f,insta:e.target.value})}/>
<input style={inp} placeholder="💳 Chave Pix" value={f.pix} onChange={e=>setF({...f,pix:e.target.value})}/>
<input style={inp} placeholder="📶 Senha WiFi ex: oficina123" value={f.wifi} onChange={e=>setF({...f,wifi:e.target.value})}/>
<input style={inp} placeholder="📍 Link Google Maps (cole o link de compartilhar do Maps)" value={f.loc} onChange={e=>setF({...f,loc:e.target.value})}/>
</div>
...
<div style={{fontSize:'9px',textAlign:'center',opacity:0.3,marginBottom:'8px'}}>👁️ PREVIEW AO VIVO</div>
...
{f.serv && <div style={{background:tema.card,padding:'12px',borderRadius:'10px',fontSize:'12px',textAlign:'center'}}>🛠️ Nossos Serviços</div>}
{f.cat && <div style={{background:tema.card,padding:'12px',borderRadius:'10px',fontSize:'12px',textAlign:'center'}}>📚 Catálogo</div>}
{f.agend && <div style={{background:tema.card,padding:'12px',borderRadius:'10px',fontSize:'12px',textAlign:'center'}}>📅 Agendar Horário</div>}
<div style={{background:tema.btn,padding:'12px',borderRadius:'10px',fontSize:'12px',fontWeight:900,textAlign:'center'}}>💬 WhatsApp {f.wpp?'':''}</div>
...
{f.loc && <div style={{background:'#0A2A5A',padding:'12px',borderRadius:'10px',fontSize:'12px',textAlign:'center',fontWeight:900}}>📍 Ver Localização no Maps</div>}
</div>
</div>
</div>
<div style={{textAlign:'center',fontSize:'9px',opacity:0.3,marginTop:'8px'}}>vision-local.com/{f.slug||'sua-loja'}</div>
</div>
</div>
</div>
)
