...
const TEMPLATES = [
  { label: "Barbearia", title: "Barbearia Style", capa: "Barbearia Premium", desc: "Corte e barba", serv: "Corte, Barba" },
  { label: "Salao", title: "Studio Beleza", capa: "Salao Completo", desc: "Escova e coloracao", serv: "Escova, Coloracao" },
  { label: "Oficina Moto", title: "Oficina do Joao", capa: "Oficina de Motos", desc: "15 anos Holambra", serv: "Revisao, Motor" },
  ...
  { label: "Pizzaria", title: "Pizzaria Sabor", capa: "Pizzaria", desc: "Pizza borda recheada", serv: "Pizzas" },
  { label: "Loja Roupas", title: "Moda Feminina", capa: "Moda Atual", desc: "Moda toda semana", serv: "Feminino" },
  { label: "Pet Shop", title: "Pet Amigo", capa: "Pet Shop", desc: "Banho e tosa", serv: "Banho" },
]
const TEMAS = [
...
export default function Criar() {
  ...
  async function salvar() {
    ...
    const { error } = await supabase.from("biosites").upsert([{ title: f.title, slug: f.slug.toLowerCase().replace(/\s+/g, "-"), whatsapp: f.wpp, instagram: f.insta, servicos: f.serv, catalogo: f.cat, pix_key: f.pix, wifi_password: f.wifi, localizacao: f.loc, logo_url: f.logo, foto_url: f.foto, capa_titulo: f.capaTitulo, capa_desc: f.capaDesc }], { onConflict: 'slug' })
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#0A0A0A', color: 'white' }}>
      <div style={{ width: '50%', padding: '14px', overflowY: 'auto', height: '100vh', borderRight: '1px solid #222' }}>
        ...
        <input style={inp} placeholder="Google Maps" value={f.loc} onChange={e => setF({...f, loc: e.target.value })} />
      ...
      <div style={{ width: '50%', background: '#050505', display: 'flex', justifyContent: 'center', padding: '20px', overflowY: 'auto', height: '100vh' }}>
        <div style={{ width: '380px' }}>
          <div style={{ background: tema.grad, borderRadius: '28px', overflow: 'hidden', animation: 'float 4s ease-in-out infinite' }}>
